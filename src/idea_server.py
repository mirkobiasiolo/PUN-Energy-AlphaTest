...
# - UN solo worker che usa la seriale; le scritture vanno in coda
...
import os, sys, time, csv, yaml, random, threading, queue, collections
...
def load_config(force=False):
    ...
    try:
        ...
        print("[CFG] File mancante:", CFG_FILE); return
...
if CFG.get("autorun", 1) == 0 and not sys.stdin.isatty():
    print("autorun=0 → uscita"); sys.exit(0)
...
def contiguous_blocks(addrs: List[int]) -> List[range]:
    ...
    for prev, cur in zip(a, a[1:]):
        if cur != prev + 1:
            blocks.append(range(start, prev + 1)); start = cur
...
# ───── Write queue (writer -> worker)
WriteCmd = Dict[str, int]  # {"ID":str,"CMD":str,"IND":int,"VAL":int}
WRITE_Q: "queue.Queue[WriteCmd]" = queue.Queue(maxsize=5000)
...
class SerialBus:
    ...
    def _apply(self):
        ...
        # Valuta "EVEN" se il device è 8E1
        ins.serial.parity   = {"NONE": serial.PARITY_NONE, "EVEN": serial.PARITY_EVEN, "ODD": serial.PARITY_ODD}[str(CFG.get("parity","NONE")).upper()]
    ...
    def open(self):
        self.instrument = minimalmodbus.Instrument(CFG["serial_port"], int(CFG.get("slave_id", 1)), mode=minimalmodbus.MODE_RTU)
        self._apply()
        try: self.instrument.serial.open()
    ...
    def _flush(self):
        try:
            ...
            try:
                self.instrument.serial.flushInput(); self.instrument.serial.flushOutput()
            except Exception: pass
    ...
    def _warmup(self):
        ...
        if addrs:
            ...
            for _ in range(int(CFG.get("startup_dummy_reads", 3))):
                try: self.instrument.read_registers(wire_0b, 1, functioncode=self.read_fcode)
                except Exception: pass
    ...
    def reopen(self):
        try:
            if self.instrument: self.instrument.serial.close()
        except Exception: pass
        self.open()
        STATS["resyncs"] += 1
        STATS["last_reset_iso"] = now_iso()

    def _gap(self): time.sleep(self.rtu_gap_ms/1000.0)

    def read_regs(self, yaml_start: int, qty: int) -> Optional[List[int]]:
        wire_0b = (yaml_start - 1) + self.addr_corr
        regs = self.instrument.read_registers(wire_0b, qty, functioncode=self.read_fcode)
        self._gap()
        return regs

    def write_reg(self, yaml_addr: int, val: int):
        wire_0b = (yaml_addr - 1) + self.addr_corr
        self.instrument.write_register(wire_0b, val, number_of_decimals=0, functioncode=6, signed=False)
        self._gap()

BUS = SerialBus(); BUS.open()

# ───── CSV
CSV_FILE = WRITER = CSV_START = None
def csv_open():
    if not CFG.get("csv_enable", True): return
    BASE_DIR.mkdir(parents=True, exist_ok=True)
    global CSV_FILE, WRITER, CSV_START
    CSV_START = dt.datetime.now()
    CSV_FILE  = (BASE_DIR / f"logfile_{CSV_START:%Y%m%d_%H%M%S}_.csv").open("w", newline="", encoding="utf-8")
...
def csv_rollover():
    ...
    if CSV_FILE and CSV_FILE.tell() >= int(CFG.get("log_rollover_bytes", 5_000_000)):
        ...
        try: CSV_FILE.close(); CSV_FILE.rename(final)
        finally: csv_open()
csv_open()
...
def io_worker():
    ...
    while True:
        try:
            ...
            # 1) Svuota la coda scritture (bounded per non affamare il read)
            max_writes = int(CFG.get("max_writes_per_cycle", 50))
                ...
                while attempts < w_retries and not ok:
                    ...
                    try:
                        ...
                        if "Wrong functioncode" in msg or "functioncode" in msg:
                            note_foreign()
                        # prova di salvataggio: leggi il registro; se il valore è già scritto, considera ok
                        try:
                ...
                for s, ln in split_range(block, max_chunk):
                    ...
                    while cur >= min_chunk:
                        ...
                        for _ in range(max_retry):
                            try:
                                regs = BUS.read_regs(s, cur); break
            ...
            if any_ok and not first_ok:
                BUS_READY.set(); first_ok = True

        except serial.SerialException as e:
            print("[BUS] SerialException:", e); BUS.reopen()
        except Exception as e:
            print("[ERR worker]", e)
...
LAST_TOIDEA_ID = ""
def toidea_watcher():
    global LAST_TOIDEA_ID
    while True:
        try:
            if not TO_FILE.exists():
                time.sleep(float(CFG.get("writer_sleep_ms", 200))/1000.0); continue
            ...
            if cmd not in ("07","09"):
                print("[WRITER] CMD non valido:", cmd); LAST_TOIDEA_ID = cid; continue
            try:
                WRITE_Q.put_nowait({"ID": cid, "CMD": cmd, "IND": ind, "VAL": val})
                LAST_TOIDEA_ID = cid
            except queue.Full:
                print("[WRITER ERR] Coda piena: comando scartato ID=", cid)
        except Exception as e:
